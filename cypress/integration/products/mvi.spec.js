...
describe('MVI', () => {
  ...
  context('Product Header', () => {
    it('should show product symbol', () => {
      cy.get('[data-cy=token-symbol]').should('contain', 'MVI')
    })
    it('should show product name', () => {
      cy.get('[data-cy=token-name]').should('contain', 'Metaverse Index')
    })
  ...
  context('My Assets', () => {
    ...
    it('should render values', () => {
      ...
      cy.get('[data-cy=my-assets-token-balance]').should('contain', 'MVI')
    })
