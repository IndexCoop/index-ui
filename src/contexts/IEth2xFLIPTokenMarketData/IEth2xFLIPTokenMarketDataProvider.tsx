...
import { IEthereum2xFLIP } from 'constants/productTokens'
...
import MarketDataContext from './IEth2xFLIPTokenMarketDataContext'

const IEth2xFLIPMarketDataProvider: React.FC = ({ children }) => {
  const [fliMarketData, setFliMarketData] = useState<any>([[]])
  const [fliMarketCap, setFliMarketCap] = useState<any>(0)

  useEffect(() => {
    fetchHistoricalTokenMarketDataOnPolygon(IEthereum2xFLIP.polygonAddress)
      .then((response: any) => {
  ...
  useEffect(() => {
    fetchSetComponentsBeta(IEthereum2xFLIP.tokensetsId)
      .then((response: any) => {
...
export default IEth2xFLIPMarketDataProvider
