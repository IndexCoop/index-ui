...
import { IMaticFLIP } from 'constants/productTokens'
...
import MarketDataContext from './IMaticFLIPTokenMarketDataContext'

const IMaticFLIPTokenMarketDataProvider: React.FC = ({ children }) => {
  const [fliMarketData, setFliMarketData] = useState<any>([[]])
  const [fliMarketCap, setFliMarketCap] = useState<any>(0)

  useEffect(() => {
    fetchHistoricalTokenMarketDataOnPolygon(IMaticFLIP.polygonAddress)
      .then((response: any) => {
  ...
  useEffect(() => {
    fetchSetComponentsBeta(IMaticFLIP.tokensetsId)
      .then((response: any) => {
...
export default IMaticFLIPTokenMarketDataProvider
